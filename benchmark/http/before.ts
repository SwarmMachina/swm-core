import { timed, type MetricsSummary, type TimedResult } from '@swarmmachina/benchkit/measurement'
import { parseArgs } from '@swarmmachina/benchkit/orchestration'
import { formatYmdHms } from '@swarmmachina/benchkit/reporting'
import { median } from '@swarmmachina/benchkit/statistics'
import { assertNonNegativeFinite, assertPositiveFinite, assertPositiveSafeInteger } from '../harness/args.js'
import runLoad from '../harness/http-load.js'
import { TargetController } from '../harness/target-controller.js'
import { TARGET_ARG_HANDLERS, targetDefaults, targetUrl } from '../harness/target-session.js'
import { REPOSITORY_ROOT, RUNTIME_BENCHMARK_ROOT } from '../harness/runtime-paths.js'
import type { HttpLoadMetrics, TargetArgs } from '../harness/types.js'

// 'plain' = native route, handler only; 'before' = same route plus a synchronous no-op hook.
const VARIANTS = ['plain', 'before'] as const

type BeforeVariant = (typeof VARIANTS)[number]

interface BeforeArgs extends TargetArgs {
  runs: number
  warmup: number
  duration: number
  connections: number
  pipelining: number
  sampleMs: number
}

/**
 * @param {string[]} argv
 * @returns {object}
 */
function parseBeforeArgs(argv: string[]): BeforeArgs {
  return parseArgs(
    argv,
    {
      runs: 4,
      warmup: 3,
      duration: 6,
      connections: 100,
      pipelining: 10,
      sampleMs: 250,
      ...targetDefaults()
    },
    {
      '--runs': (out, v) => {
        out.runs = Number(v)
      },
      '--warmup': (out, v) => {
        out.warmup = Number(v)
      },
      '--duration': (out, v) => {
        out.duration = Number(v)
      },
      '--connections': (out, v) => {
        out.connections = Number(v)
      },
      '--pipelining': (out, v) => {
        out.pipelining = Number(v)
      },
      '--sample-ms': (out, v) => {
        out.sampleMs = Number(v)
      },
      ...TARGET_ARG_HANDLERS
    }
  ) as BeforeArgs
}

function validateBeforeArgs(args: BeforeArgs): void {
  assertPositiveSafeInteger(args.runs, '--runs')
  assertNonNegativeFinite(args.warmup, '--warmup')
  assertPositiveFinite(args.duration, '--duration')
  assertPositiveSafeInteger(args.connections, '--connections')
  assertPositiveSafeInteger(args.pipelining, '--pipelining')
  assertPositiveSafeInteger(args.sampleMs, '--sample-ms')
}

interface BeforeRun {
  variant: BeforeVariant
  rps: number
  latAvgMs: number | null
  latP97_5Ms: number | null
  latP99Ms: number | null
  errors: number
  rssMB: number | null
  heapMB: number | null
  cpuCorePct: number | null
  eluPct: number | null
}

interface BeforeRunOptions {
  variant: BeforeVariant
  warmupSec: number
  durationSec: number
  connections: number
  pipelining: number
  runIndex: number
  sampleMs: number
  runStamp: string
  targetController: TargetController
}

/**
 * @param {object} params
 * @param {string} params.variant
 * @param {number} params.warmupSec
 * @param {number} params.durationSec
 * @param {number} params.connections
 * @param {number} params.pipelining
 * @param {number} params.runIndex
 * @param {number} params.sampleMs
 * @param {string} params.runStamp
 * @param {object} params.targetController
 * @returns {Promise<object>}
 */
async function runOne({
  variant,
  warmupSec,
  durationSec,
  connections,
  pipelining,
  runIndex,
  sampleMs,
  runStamp,
  targetController
}: BeforeRunOptions): Promise<BeforeRun> {
  const { session } = await targetController.start({
    serverName: 'http/before-target.js',
    fw: variant,
    testName: 'before',
    runIndex,
    v8prof: false,
    runStamp
  })
  const url = targetUrl('http', session, '/')
  const baseOpts = {
    method: 'GET',
    url,
    duration: durationSec,
    connections,
    pipelining
  }

  let runTimed: TimedResult<HttpLoadMetrics> | undefined
  let m: MetricsSummary | null | undefined

  try {
    if (warmupSec > 0) {
      await runLoad(`${variant}-warmup`, { ...baseOpts, duration: warmupSec })
    }

    await session.startMetrics({ sampleMs })
    runTimed = await timed(() => runLoad(variant, baseOpts))
    m = await session.stopMetrics()
  } finally {
    await session.stop()
  }

  if (!runTimed) {
    throw new Error(`${variant}: measurement did not complete`)
  }

  const r = runTimed.result

  return {
    variant,
    rps: r.rps,
    latAvgMs: r.latencyAvgMs,
    latP97_5Ms: r.latencyP97_5Ms,
    latP99Ms: r.latencyP99Ms,
    errors: r.errors,
    rssMB: m?.memMB?.rssPeak ?? null,
    heapMB: m?.memMB?.heapUsedPeak ?? null,
    cpuCorePct: m?.cpuCorePct ?? null,
    eluPct: m?.eluPct ?? null
  }
}

/**
 *
 */
async function main() {
  const args = parseBeforeArgs(process.argv)

  validateBeforeArgs(args)

  const targetController = new TargetController(args, REPOSITORY_ROOT, RUNTIME_BENCHMARK_ROOT)
  const runStamp = formatYmdHms()
  const per: Record<BeforeVariant, BeforeRun[]> = {
    plain: [],
    before: []
  }

  console.log(
    `Run before-hook bench: variants:${VARIANTS.join(',')}, connections:${args.connections}, ` +
      `pipelining:${args.pipelining}, duration:${args.duration}, runs:${args.runs}`
  )

  for (let i = 0; i < args.runs; i++) {
    console.log(`\n== run ${i + 1}/${args.runs} ==`)

    const order = i % 2 === 0 ? VARIANTS : VARIANTS.slice().reverse()

    for (const variant of order) {
      const row = await runOne({
        variant,
        warmupSec: args.warmup,
        durationSec: args.duration,
        connections: args.connections,
        pipelining: args.pipelining,
        runIndex: i,
        sampleMs: args.sampleMs,
        runStamp,
        targetController
      })

      per[variant].push(row)

      console.log(
        `[before-bench] ${variant}: rps=${Math.round(row.rps)} ` +
          `p99=${row.latP99Ms != null ? row.latP99Ms.toFixed(2) : 'n/a'}ms ` +
          `heap=${row.heapMB != null ? row.heapMB.toFixed(0) : 'n/a'}MB errors=${row.errors}`
      )
    }
  }

  const medians = VARIANTS.map((variant) => {
    const arr = per[variant]
    const med = (key: Exclude<keyof BeforeRun, 'variant'>): number | null => {
      const values = arr.map((row) => row[key]).filter((value): value is number => value !== null)

      return values.length ? median(values) : null
    }
    const round = (value: number | null, digits: number): number | null =>
      value === null ? null : Number(value.toFixed(digits))
    const rps = med('rps')

    return {
      variant,
      rps: rps === null ? null : Math.round(rps),
      latAvgMs: round(med('latAvgMs'), 3),
      latP97_5Ms: round(med('latP97_5Ms'), 3),
      latP99Ms: round(med('latP99Ms'), 3),
      rssMB: round(med('rssMB'), 1),
      heapMB: round(med('heapMB'), 1),
      cpuCorePct: round(med('cpuCorePct'), 1),
      eluPct: round(med('eluPct'), 1)
    }
  })

  console.log('\n== median ==')
  console.table(medians)

  const plain = medians.find((m) => m.variant === 'plain')
  const before = medians.find((m) => m.variant === 'before')

  if (plain?.rps && before?.rps) {
    const rpsDelta = ((before.rps - plain.rps) / plain.rps) * 100
    const heapDelta = plain.heapMB != null && before.heapMB != null ? before.heapMB - plain.heapMB : null

    console.log(
      `\nbefore-hook overhead vs plain: rps ${rpsDelta >= 0 ? '+' : ''}${rpsDelta.toFixed(1)}%` +
        (heapDelta != null ? `, heap ${heapDelta >= 0 ? '+' : ''}${heapDelta.toFixed(1)}MB` : '')
    )
  }
}

main().catch((e) => {
  console.error(e)
  process.exitCode = 1
})
