...
import { requireFiniteNumber, type ProfiledBenchmarkSummary, type SuiteOptions } from '../../harness/types.js'
...
export default async function runWsUpgradeSuite({ sourceBenchDir, runtimeBenchDir, repoRoot, outRoot }: SuiteOptions) {
  const baseline = await readJson<WsUpgradeBaseline>(
    path.join(sourceBenchDir, 'regression', 'baselines', 'ws-upgrade.json')
  )
  ...
  await runChild([
    path.join(runtimeBenchDir, 'ws', 'upgrade.js'),
    '--fw',
    'core',
    '--scenario',
    SCENARIOS.join(','),
