...
function numEnv(name: string, fallback: number, env: NodeJS.ProcessEnv = process.env): number {
...
export default async function runHttpSuite({ sourceBenchDir, runtimeBenchDir, repoRoot, outRoot }: SuiteOptions) {
  const baseline = await readJson<HttpBaseline>(path.join(sourceBenchDir, 'baselines', 'http.json'))
