 ...
 * @returns {object}
 */
function parsePayloadArgs(argv) {
  return parseArgs(
    argv,
    {
...
async function main() {
  ...
  const outDir = args.outDir || path.join(__dirname, 'profiles', 'ws-payload')
  const rows = []
  ...
  for (const size of args.sizes) {
    ...
    const result = JSON.parse(await fs.readFile(jsonOut, 'utf8'))
    ...
    for (const framework of args.frameworks) {
      ...
      const metricMedian = (key) => finiteMedian(runRows.map((item) => item[key]))
