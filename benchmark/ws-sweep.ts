...
interface SweepBenchmarkRow {
  fw: string
  msgPerSec?: number | null
  latP99Ms?: number | null
}

interface SweepBenchmarkSummary {
  median: SweepBenchmarkRow[]
}
...
async function main() {
  ...
  const table: Array<Record<string, number | string | null>> = []
  ...
  for (const connections of args.connectionsList) {
    ...
    const bench = await readJson<SweepBenchmarkSummary>(jsonOut)
    const row: Record<string, number | string | null> = { connections }

    for (const fw of frameworks) {
      const median = bench.median.find((r) => r.fw === fw)

      row[`${fw} msg/s`] = median?.msgPerSec == null ? null : Math.round(median.msgPerSec)
