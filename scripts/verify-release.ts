 ...
 * @returns {string|null}
 */
export function getBindingLockIntegrity({ manifest, lockfile }: BindingLockParams): string | null {
 ...
 * @param {string} params.lockfile
 */
export function verifyBindingLockIntegrity({ manifest, lockfile }: BindingLockParams): void {
  const name = '@swarmmachina/swm-uws'
  const version = manifest.dependencies?.[name]
...
export async function verifyRepositoryRelease(tag: string | undefined): Promise<{
  ...
  const [manifest, lockfile] = await Promise.all([
    fs.readFile(path.join(ROOT, 'package.json'), 'utf8').then((text) => JSON.parse(text) as ReleaseManifest),
