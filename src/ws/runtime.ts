...
import { validateWsClose } from './protocol.js'
import WebSocketUpgradeRuntime, { WS_CONTEXT_DATA } from './upgrade.js'

import type { RequestPrefetchPlan, WebSocket } from '@swarmmachina/swm-uws'
import type { HeaderPrefetch } from '../server/options.js'
import type WebSocketUpgradeMeta from './upgrade-meta.js'
...
export default class WebSocketRuntime {
  ...
  #invokeCallback<Args extends unknown[]>(
    ctx: WSContext,
    callback: (ctx: WSContext, ...args: Args) => unknown,
    ...args: Args
  ): void {
    try {
      ...
      if (isPromise(result)) {
        void Promise.resolve(result).catch((err) => this.#server.safeCall(this.#server.onWsError, ctx, err))
      }
    } catch (err) {
      void this.#server.safeCall(this.#server.onWsError, ctx, err)
    }
  }
  ...
  onOpen = (ws: NativeWebSocket): void => {
    ...
    this.#invokeCallback(ctx, server.onWsOpen)
  }
  ...
  onMessage = (ws: NativeWebSocket, message: ArrayBuffer, isBinary: boolean): void => {
    ...
    this.#invokeCallback(ctx, server.onWsMessage, message, isBinary)
  }
  ...
  onDropped = (ws: NativeWebSocket, message: ArrayBuffer, isBinary: boolean): void => {
    ...
    this.#invokeCallback(ctx, server.onWsDropped, message, isBinary)
  }
  ...
  onSubscription = (ws: NativeWebSocket, topic: ArrayBuffer, newCount: number, oldCount: number): void => {
    ...
    this.#invokeCallback(ctx, server.onWsSubscription, topic, newCount, oldCount)
  }
  ...
  onDrain = (ws: NativeWebSocket): void => {
    ...
    this.#invokeCallback(ctx, server.onWsDrain)
  }
  ...
  onClose = (ws: NativeWebSocket, code: number, message: ArrayBuffer): void => {
    ...
    try {
      result = server.onWsClose(ctx, code, message.slice(0))
