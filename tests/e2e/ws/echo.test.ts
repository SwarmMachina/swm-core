...
test('ws context keeps the exact object returned by onUpgrade', async () => {
  ...
  let openedData: Readonly<{ userId: number }> | null = null

  handle = await startWsServer({
    ws: {
      onUpgrade: () => dataFromOnUpgrade,
      onOpen: (ctx) => {
        openedData = ctx.data as Readonly<{ userId: number }>
      }
    }
  })
...
test('async ws upgrade reads owned metadata after await', async () => {
  const opened = { data: null as { url: string; query: string | undefined; header: string } | null }
  ...
  handle = await startWsServer({
    ws: {
      ...
      onOpen: (ctx) => {
        opened.data = ctx.data as { url: string; query: string | undefined; header: string }
      }
    }
  })
  ...
  if (opened.data === null) {
    throw new Error('Expected opened WebSocket data')
  }
...
test('maxPayloadLength accepts exact text/binary messages and rejects one byte over', { timeout: 5000 }, async () => {
  const received: Array<{ length: number; isBinary: boolean }> = []
