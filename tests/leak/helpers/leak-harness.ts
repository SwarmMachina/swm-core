 ...
 */
export function makeMarker(id: number): { id: number; blob: Buffer } {
 ...
 * @returns {Promise<void>}
 */
export async function assertCollected(refs: WeakRef<object>[], label: string): Promise<void> {
  ...
  const alive = refs.reduce((count, ref) => (ref.deref() === undefined ? count : count + 1), 0)
...
export async function assertNoMemoryGrowth(
  iterate: (index: number) => Promise<void>,
  {
    warmup = 50,
    iterations = 200,
    maxGrowthBytes = 1.5 * 1024 * 1024,
    label
  }: { warmup?: number; iterations?: number; maxGrowthBytes?: number; label: string }
) {
