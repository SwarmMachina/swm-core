...
import type { HttpServerHandle, WsServerHandle } from '../helpers/e2e-server.js'
...
for (const scenario of makeHttpScenarios()) {
  ...
  const handle: HttpServerHandle = await startHttpServer(
    scenario.serverOptions(noop) as Parameters<typeof startHttpServer>[0]
  )

  entries.push({
    name: scenario.name,
    run: (iteration) => scenario.run(handle, noop, iteration),
...
for (const scenario of makeWsScenarios()) {
  ...
  const handle: WsServerHandle = await startWsServer(scenario.serverOptions(noop))
...
console.log(`soak: duration=${values.duration}s scenarios=${entries.map((entry) => entry.name).join(',')}`)
