...
describe('BodyBudget', () => {
  ...
  test('rejects invalid limits and reservation sizes without coercion', () => {
    for (const value of [-1, 1.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1, '1', {}, new Number(1)]) {
      throws(() => {
        // @ts-expect-error -- validates runtime rejection of non-number limits.
        new BodyBudget(value)
      }, TypeError)
    }
    ...
    for (const value of [-1, 1.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1, '1', {}]) {
      throws(() => {
        // @ts-expect-error -- validates runtime rejection of non-number reservations.
        budget.tryReserve(value, {})
      }, TypeError)
    }
  })
})
